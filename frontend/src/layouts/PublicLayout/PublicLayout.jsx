...
export default function PublicLayout() {
  ...
  const SOCIAL_LINKS = [
    ...
    {
      href: 'https://t.me/dentuz_uz', label: 'Telegram', tag: 'UZ',
      svg: <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 0 0-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.95-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.36.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .38z"/></svg>
    },
    {
      href: 'https://wa.me/998712004545', label: 'WhatsApp', tag: 'WA',
      svg: <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21 5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.816 9.816 0 0 0 12.04 2m.01 1.67c2.2 0 4.26.86 5.82 2.42a8.225 8.225 0 0 1 2.41 5.83c0 4.54-3.7 8.24-8.24 8.24-1.48 0-2.93-.4-4.2-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.196 8.196 0 0 1-1.26-4.38c0-4.54 3.7-8.24 8.24-8.24M8.53 7.33c-.16 0-.43.06-.66.31-.22.25-.86.84-.86 2.05 0 1.21.88 2.38 1 2.55.13.16 1.74 2.65 4.21 3.72.59.25 1.05.4 1.41.51.59.19 1.13.16 1.56.1.48-.07 1.47-.6 1.68-1.18.21-.58.21-1.07.15-1.18-.06-.1-.23-.16-.48-.29s-1.47-.73-1.7-.81c-.23-.08-.4-.13-.57.13-.17.25-.66.81-.81.98-.15.16-.3.18-.55.06s-1.05-.39-2-1.23c-.74-.66-1.23-1.47-1.38-1.72-.15-.25-.02-.38.11-.5.11-.11.25-.29.38-.43.13-.15.17-.25.25-.42.08-.17.04-.32-.02-.45s-.57-1.37-.78-1.88c-.2-.49-.41-.43-.57-.44-.15-.01-.32-.01-.49-.01z"/></svg>
    },
  ];
  ...
  return (
    <div className={styles.publicContainer}>
      ...
      ══════════════════════════════════════════ */}
      {/* ══════════════════════════════════════════
          GATEDENT & APPLE STYLE 5-COLUMN FOOTER
      ══════════════════════════════════════════ */}
      <footer
        ref={footerRef}
        ...
        <div className={styles.footerInner}>
          ...
          >
            ...
            <div className={styles.brandCol}>
              ...
              >
                ...
                <div className={styles.itParkVoteBadge}>
                  <span className={styles.itParkVoteArrow}>▲</span>
                  <span className={styles.itParkVoteCount}>1</span>
                </div>
              </a>

              {/* Social Media Row */}
              <div className={styles.socialRow}>
                {SOCIAL_LINKS.map(({ href, label, tag, svg }) => (
                  <a
                    key={label}
                    href={href}
                    target="_blank"
                    rel="noreferrer"
                    className={`${styles.socialBtn} ${styles['social_' + label.toLowerCase()]}`}
                    aria-label={label}
                    title={label}
                  >
                    {tag && <span className={styles.socialCountryTag}>{tag}</span>}
                    {svg}
                  </a>
                ))}
              </div>
            ...
            <div className={styles.footerCol}>
              ...
              <a href="mailto:info@dentuz.uz" className={styles.officeEmailLink}>
                info@dentuz.uz
              </a>
            </div>
          ...
          {/* ── Trust Badges Row (GateDent style) ── */}
          <div
          ...
          >
            {TRUST_ITEMS.map(({ icon, label }) => (
              <div key={label} className={styles.trustPill}>
                <Icon name={icon} size={15} className={styles.trustIcon} />
                <span>{label}</span>
              </div>
            ))}
          </div>
